...
- **Phase 4**（検証済み）：`user_assignment`5件（うち3件提出済み）のフィクスチャに対しダッシュボードが3件提出済み・2件未提出を正しく判定することを結合テスト（`backend/internal/assignment/service_integration_test.go`）で確認。事務局管理者が全社横断で提出のバリデーション状態を確認できることを`backend/internal/inputsheet/handlers.go`の`ListForReviewHandler`と結合テストで確認。ログイン/ログアウト、ディメンションマスタCRUD、シナリオ版作成、fact入力、シート作成・更新、バインディング作成、提出、実績インポートというPhase 1〜3の全操作について、対応する`(action, entity_type)`の組み合わせで`audit_log`に最低1行ずつ存在することを、実際のHTTPリクエストでアプリ全体を駆動する結合テスト（`backend/internal/httpapi/phase4_audit_trail_integration_test.go`）で確認した。`submission_scope`と`audit_log`の書き込み条件は非対称であることをテストで明示的に確認済み：バリデーションエラーで`fact_amount`/`submission_scope`が書き込まれない提出でも、提出という行為自体は`audit_log`に必ず記録される（`backend/internal/inputsheet/submission_scope_integration_test.go`）。ブラウザでの手動E2Eで、ディメンションマスタ作成→担当割当て作成→シート提出→提出状況ダッシュボードへの反映（未提出/提出済み/バリデーション結果の表示）、ラベル変更による再提出エラー時にダッシュボードの表示が直前の成功提出のまま保持されること、監査ログ画面でのフィルタ・ページネーション、`field_user`アカウントでは4画面のナビ・クイックリンクが非表示になり直接URLアクセスでもクライアント側ガード＋API側403（`adminOnly`ミドルウェア）の両方が機能することを確認した。
