...
- **スプレッドシートエンジン**：Univer.js を採用（決定済み）。Phase 2着手時にFacade API（`createUniver`/`FRange.getDisplayValues()`/`FWorkbook.save()`等）で選択範囲の座標取得・セルテキスト抽出・ワークブックJSONシリアライズの一連の流れをスパイクし、致命的な問題がなかったため採用を確定、Fortune-Sheetの試作は不要と判断した。**実装上の注意**：`@univerjs/presets`パッケージ（`createUniver`/`LocaleType`/`merge`の提供元）をそのままインストールすると、その依存関係に非公開ライセンスの`@univerjs-pro/*`パッケージが大量に連鎖的についてくる（`@univerjs/preset-docs-advanced`経由）。これはビルド後のバンドルには含まれない（未importのため）が、node_modules上の衛生・ライセンス監査の観点から避けたいので、`@univerjs/preset-sheets-core`（sheets限定プリセット、pro依存なし）と`@univerjs/presets`の**メインエントリのみ**（`createUniver`等はcoreからも間接的に再エクスポートされている）を組み合わせて使用する。また`@univerjs/core`が依存する`nanoid`に既知の脆弱性（GHSA-28wg-ghj8-5hjv、非セキュアな乱数生成のDoS系）があったため、`frontend/package.json`の`overrides`で`nanoid@^5.1.16`に固定して解消済み（`npm audit`は0件）。**フロントエンドのコード分割**：Univer関連のバンドル（数式エンジンのロケールデータを含め数MB）はログイン画面等の無関係なページでは読み込まれないよう、`SheetEditorPage`を`React.lazy`でルート単位に分割している。
...
- **Phase 2**（検証済み）：現場担当者がシートを作成しバインディングを定義して提出し、結果の`fact_amount`行がシートのグリッド値＋バインディング設定から算出した値と厳密に一致することを結合テスト（`backend/internal/inputsheet/submission_integration_test.go`）で確認。構造を変えず値だけ変更しての再提出がUPSERTで正しく処理されることも確認。バインディング済みの行ラベルを変更して再提出すると`validation_status=error`となり、`fact_amount`が一切書き換わらない（＝誤ったデータでの上書きを防ぐ）ことをテストで確認。ブラウザでの手動E2E確認（シート作成→データ入力→範囲選択→バインディングウィザード→提出→予実差異レポートへの反映）も実施し、Phase 1の簡易フォームで入力した実績と同じ`fact_amount`テーブル上で予算/実績が並んで表示されることを確認した。MVPスコープの簡略化として、バインディングウィザードは`header_rows=1`・`header_cols=1`（単一の見出し行・見出し列）固定とし、複数行/列の階層見出しはUIとしては提供していない（スキーマ上は将来拡張可能）。
- **Phase 3**（検証済み）：サンプルのCSVをアップロードして正しい`actual`ファクト行が生成され予実差異レポートに反映されることを結合テスト（`backend/internal/importer/service_integration_test.go`）およびブラウザでの手動E2Eで確認。列マッピングが一部の行で解決できない場合（存在しない勘定科目名など）も、解決できる行だけ`completed`ステータスで部分成功し、`import_batch.error_count`とエラー詳細が正しく記録されることを確認。content-type/マジックバイト不一致・空ファイル・行数上限超過・拡張子と実体の不一致を単体テストで拒否を確認、XLSXのzip展開サイズ上限（`excelize.Options.UnzipSizeLimit`）が実際に機能することも単体テストで確認（本物のzip爆弾フィクスチャを用意する代わりに、正常な小さなXLSXに対して極端に小さい上限を与えて拒否されることを検証——配線が正しく機能していることの確認が目的で、excelize自体のzip爆弾耐性はライブラリ側の責務として信頼している）。CSV/Excelエクスポートは日本の会計システムに多いShift-JIS想定でデコードを実装・確認。予実差異レポートのドリルダウンは、バックエンドAPIを変更せずフロントエンド側で期間ごとの行を（事業×部門×勘定科目）単位に集計し、クリックで元の期間別明細行を展開する形で実装・確認。

  **手動E2Eで発見・修正したバグ**：(1) `ColumnMapping`構造体にJSON構造体タグがなく、フロントエンドが送るスネークケースのフィールド名と一致せず、列インデックスが常にゼロ値にサイレントに解決されていた（ユニットテスト・結合テストはどちらもGo構造体を直接組み立てて`Commit`を呼んでいたため、この不整合はJSONを実際に経由するHTTPハンドラ経由のテストでしか発見できなかった。教訓を反映し`handlers_test.go`にJSON経由の回帰テストを追加）。(2) `error_detail`/`validation_detail`のようなNULL許容JSON列をNULLのまま書き込むと、`database/sql`が`*json.RawMessage`へのNULL Scanに対応しておらず読み出し時にエラーになる問題を発見し、常に有効なJSON（値がない場合は`null`リテラル）を書き込むよう修正。
- **Phase 4**（検証済み）：`user_assignment`5件（うち3件提出済み）のフィクスチャに対しダッシュボードが3件提出済み・2件未提出を正しく判定することを結合テスト（`backend/internal/assignment/service_integration_test.go`）で確認。事務局管理者が全社横断で提出のバリデーション状態を確認できることを`backend/internal/inputsheet/handlers.go`の`ListForReviewHandler`と結合テストで確認。ログイン/ログアウト、ディメンションマスタCRUD、シナリオ版作成、シート作成・更新、バインディング作成、提出、実績インポートというPhase 1〜3の全操作について（手動fact入力は本ブランチのベース後に廃止されたため対象から除外）、対応する`(action, entity_type)`の組み合わせで`audit_log`に最低1行ずつ存在することを、実際のHTTPリクエストでアプリ全体を駆動する結合テスト（`backend/internal/httpapi/phase4_audit_trail_integration_test.go`）で確認した。`submission_scope`と`audit_log`の書き込み条件は非対称であることをテストで明示的に確認済み：バリデーションエラーで`fact_amount`/`submission_scope`が書き込まれない提出でも、提出という行為自体は`audit_log`に必ず記録される（`backend/internal/inputsheet/submission_scope_integration_test.go`）。ブラウザでの手動E2Eで、ディメンションマスタ作成→担当割当て作成→シート提出→提出状況ダッシュボードへの反映（未提出/提出済み/バリデーション結果の表示）、ラベル変更による再提出エラー時にダッシュボードの表示が直前の成功提出のまま保持されること、監査ログ画面でのフィルタ・ページネーション、`field_user`アカウントでは4画面のナビ・クイックリンクが非表示になり直接URLアクセスでもクライアント側ガード＋API側403（`adminOnly`ミドルウェア）の両方が機能することを確認した。

  **実装中に見つけた設計修正**：(1) 当初`docs/plan.md`のPhase 4スキーマ草案では`submission`テーブルに`scenario_version_id`用の新規インデックスを追加する想定だったが、実際にはPhase 2のFK制約（`FOREIGN KEY (scenario_version_id) REFERENCES scenario_version(id)`）作成時にInnoDBが既に暗黙のインデックスを自動生成していたため不要と判明し、マイグレーションから削除した（`SHOW CREATE TABLE submission`で確認）。(2) 同じ理由で、一度このインデックスを明示的に追加してしまうとInnoDBがFKの裏付けインデックスをそちらに差し替えるため、goose downで単純に`DROP INDEX`しようとすると「needed in a foreign key constraint」エラーで失敗することを、実際に`up→down→up`のマイグレーションサイクルをローカルMySQLコンテナで検証して発見した——インデックス追加自体をやめたことでこの罠を回避した。
...
## UI/UXデザイン刷新（サイドバーナビゲーション化）（検証済み）

Phase 3完了後、ユーザーから「社内ツールとはいえ使いたくなるUIにしたい」「PC利用前提で、最近よくあるメニューペイン付きのデザインが良い」「中央に寄せすぎると紙面が勿体無い」との要望があり、業務ロジック・データモデル・APIには一切手を加えず`frontend/src/**`のみを対象にレイアウト刷新を行った。

**実施内容**：
- `lucide-react`を新規依存として追加し、サイドバーナビゲーション用アイコンに使用。
- 新規`frontend/src/components/AppShell.tsx`：左サイドバー（ナビリンク＋`NavLink`のアクティブ状態ハイライト、役割別ガード、ユーザー情報＋ログアウト）＋`<Outlet/>`によるメインコンテンツ領域。折りたたみ（フル幅⇄アイコンのみ）はデスクトップ幅（`lg`=1024px以上）で、`lg`未満ではオフキャンバスドロワー（ハンバーガー＋バックドロップ）に切り替わる。
- `frontend/src/routes.tsx`をネストルート化し、`AppShell`を親のレイアウトルートとして全ページ（`/login`を除く）を子ルートに統合。個別の`<ProtectedRoute>`ラップを撤廃。
- 全ページから個別の`mx-auto mt-{N} max-w-{X} px-4`ラッパーと「← ダッシュボード」戻るリンクを撤去（`SheetEditorPage`の「← マイシート一覧」リンクのみ、シート固有の文脈的ナビゲーションとして維持）。幅・余白は`AppShell`側の`pageContent`（`frontend/src/lib/ui.ts`、`max-w-[1600px]`）に一本化。
- `DashboardPage`をリンクの`<ul>`からアイコン付きクイックアクセスカード（`card`クラス）に再設計。
- `frontend/src/index.css`のアクセントカラーを紫（`#6a2bb8`/ダーク`#c084fc`）からインディゴ系（`#4f46e5`/ダーク`#818cf8`）に変更し、サイドバー専用トークン（`--color-sidebar`/`--color-sidebar-border`）を新設。ダークモードは既存踏襲で`prefers-color-scheme`駆動のまま。
